...
To install every configuration file, run:

#+begin_src bash
./install.ml
#+end_src

The script runs through its shebang, so there is no build step. To act on a single
directory, pass an action and a name. The action is a bare letter, not a flag:

#+begin_src bash
./install.ml i emacs   # install only emacs
./install.ml u emacs   # uninstall emacs
./install.ml c emacs   # run the configure script for emacs
#+end_src

Which file goes where is defined in ~dest_places.txt~, one entry per line in the format
~src:dest[:symlink|hardlink][:chmod]~, where ~dest~ is relative to ~$HOME~.
